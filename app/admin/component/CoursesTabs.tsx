    ...
    return (
        <>
            ...
            <Tabs defaultValue={courses[0]?.title || ""} className="">
                ...
                <TabsList className="flex items-end border-b border-[#F0EAE8] overflow-x-auto no-scrollbar">
                    {courses.map((course, index) => (
                        <TabsTrigger
                            key={index}
                            value={course.title}
                            onClick={() => handleChange(index)}
                            className={`px-6 py-2 text-lg text-[#602712] cursor-pointer flex gap-2 items-center whitespace-nowrap ${
